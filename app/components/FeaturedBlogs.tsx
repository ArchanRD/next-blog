...
export default function FeaturedBlogs({ data }: Props) {
  return (
    <Link href={`blog/${data?.slug?.current}`}>
      <div className="md:rounded-2xl rounded-xl lg:h-[450px] w-85 mx-auto md:w-full md:mx-0 md:h-[300px] h-96 bg-white flex flex-col md:flex-row">
        <div className="md:h-full relative md:w-3/5 lg:w-9/12 h-1/2">
          <Image
            src={data?.mainImage.asset.url}
            layout="fill"
            objectFit="cover"
            className="md:rounded-s-2xl md:rounded-tr-none rounded-t-xl w-full"
            alt="image"
          />
        </div>
